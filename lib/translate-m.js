...
program
  ...
  .option('-t, --target [language]', 'Target language [es]', defaultTarget)
  .on('--help', () => {
    console.log('\nExamples:');
    console.log("\t$ translate-m 'I want to translate this text'");
    console.log("\t$ translate-m -s es -t en 'Quiero traducir este texto'");
    console.log("\t$ translate-m -s en -t es I want to translate this text");
    console.log("\t$ translate-m -a 'Au revoir' -d");
    console.log("\t$ pbpaste | translate-m # when text stored in Mac clipboard");
  })
  .parse(process.argv);
...
async function runTranslation(sourceText) {
  const sourceLang = (program.auto) ? 'auto' : program.source;
  const targetLangs = program.target.split(',');
  var counter = 1;
  targetLangs.forEach(async targetLang => {
    ...
    try {
      ...
      const response = await requestPromise(targetURL);
